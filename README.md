...
| **Пояснительная записка** | Выжимка через GigaChat (`/api/gigachat/simple`, `maxTokens: 100000`); формат разделов 1.1–1.10.8 как в ИГИ; вставляется **перед** медиа |
...
| **Нумерация страниц** | Сквозная нумерация по центру нижнего колонтитула (поле PAGE) |
| **Фоновая склейка** | Длительная операция в `QThread`, UI не блокируется |

### Результат работы

- **`{имя}.docx`** и **`{имя}.pdf`** сохраняются в **выбранной папке**
- В DOCX: склеенный отчёт → **пояснительная записка** → приложения с фото/чертежами
